"""Unit tests for CommercialIntakeAgent's ToolCallingOrchestrator injection (PBI-12-04,
generalizing PBI-02-04's ClaimsAgent-only wiring — see
docs/Architecture/adr/0011-react-pattern-for-tool-orchestrated-reasoning.md): the LLM can
request one of CommercialIntakeAgent's allow-listed Tools and the outcome is surfaced as typed
AgentResponse.tool_calls, an LLM-requested Tool outside the allow-list is safely rejected, the
deterministic business-fact text/CommercialIntakeState are never affected either way, and the
...
and assertions for the third Agent this pattern was generalized to.
"""
...
from src.llm.models import LLMRequest, LLMResponse, ToolCallArgument, ToolCallRequest
...
def _build_full_commercial_tool_registry() -> InMemoryToolRegistry:
    """Registers every Tool src.core.tool_calling.policies.COMMERCIAL_ALLOWED_TOOLS names —
    required because CommercialIntakeAgent always builds tool definitions for its full
    allow-list every turn."""
...
async def test_agent_surfaces_a_successful_llm_requested_tool_call() -> None:
    tool_registry = _build_full_commercial_tool_registry()
    agent = _build_agent(
        tool_registry,
        tool_call_plan=[
            ToolCallRequest(
                ...
                arguments=[
                    ToolCallArgument(name="company_name", value="Acme Consulting LLC"),
                    ToolCallArgument(name="contact_name", value="Jane Doe"),
                    ToolCallArgument(name="contact_email", value="jane@example.com"),
                    ToolCallArgument(name="insurance_line", value="general liability"),
                    ToolCallArgument(name="risk_description", value="Consulting services."),
                ],
            )
        ],
    )
    ...
    assert response.tool_calls[0].tool_name == "lead_registration"


async def test_agent_never_offers_or_executes_a_tool_outside_its_allow_list() -> None:
    """policy_lookup is a real, registered Tool — just not in
    src.core.tool_calling.policies.COMMERCIAL_ALLOWED_TOOLS (CommercialIntakeAgent never
    queries policy/broker/claims data, CLAUDE.md §2). Even when the (scripted, test-only) LLM
    is instructed to request it, this Agent's own allow-list means it is never among the tool
    definitions offered to the LLM in the first place."""
    tool_registry = _build_full_commercial_tool_registry()
...
async def test_tool_calling_never_alters_the_deterministic_business_fact_text() -> None:
    ...
    (CLAUDE.md §3)."""
    tool_registry = _build_full_commercial_tool_registry()
    message = "I'd like a quote for new business coverage"
...
async def test_tool_calling_error_degrades_gracefully_without_blocking_the_turn() -> None:
    ...
    block the turn — CommercialIntakeAgent degrades to empty tool_calls, exactly like
    ClaimsAgent."""
    empty_registry = InMemoryToolRegistry()  # lead_registration not registered
...
class _ReasoningLeakingLLMProvider:
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        return LLMResponse(
            text=(
                "Thought: I should call lead_registration once I have every required field. "
                "Observation: none yet. Final Answer: ask for the company name."
            ),
            model="stub",
        )
