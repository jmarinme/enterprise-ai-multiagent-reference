...
- [x] PBI-04-03: Fix Azure OpenAI Tool Calling message sequencing (resolves PBI-04-02's discovered defect).
...
| AC-25 | Root cause explained before any code change (execution sequence, exact protocol violation, why MockLLMProvider never exposed it, minimum fix) | Conversation transcript; `decisions.md`'s PBI-04-03 entry |
| AC-26 | `LLMMessage` gains the minimum field needed (`tool_calls` on ASSISTANT messages); `ToolCallingOrchestrator` appends it before the TOOL result message(s), provider-agnostically | `src/llm/models.py`, `src/core/tool_calling/orchestrator.py` |
| AC-27 | No provider-specific hacks; no Azure-only special-casing — every provider's mapping updated consistently | `src/llm/azure_openai_provider.py`, `src/llm/ollama_provider.py` both updated; `src/llm/mock_provider.py` needed zero changes |
| AC-28 | Supervisor, ClaimsAgent/BrokerAgent/CommercialAgent logic, PromptManager, RAG, Grounding, citations, Tool allow-lists, conversation/correlation/user IDs, max-iteration protection all preserved unchanged | Code review — `git status` shows zero changes outside `src/llm/`, `src/core/tool_calling/orchestrator.py`, and their tests; full regression suite green |
| AC-29 | Tests cover: assistant tool_calls persisted, tool_call_id linkage, multiple tool calls + ordering, unauthorized/malformed/iteration-protection regression, MockLLMProvider/AzureOpenAIProvider/Ollama regression, Claims/Broker/Commercial regression | `tests/unit/core/tool_calling/test_tool_calling_orchestrator.py` (17 tests, 2 new), `tests/unit/llm/test_azure_openai_provider.py` (+3), `tests/unit/llm/test_ollama_provider.py` (+1), full agent suites green |
| AC-30 | Only `ca-tmxap-dev-api` rebuilt/redeployed; Web/Cosmos/Azure OpenAI/Key Vault/Managed Identity/Container Apps Environment/ACR/networking untouched | `validation.md` — single `az acr build` + `az containerapp update` on the API app only; Web revision confirmed unchanged |
| AC-31 | Live: Azure OpenAI accepts the full tool-calling sequence; Claims multi-turn works from the deployed API; Broker/Commercial still work; no regression | `validation.md` — real 10-turn Claims conversation reaching a genuine claim reference; Broker/Commercial 2-turn regression confirmed |
...
PBI-04-03: Fixed the exact defect PBI-04-02 discovered and was forbidden from touching. **Root cause** (fully explained before any code change): `ToolCallingOrchestrator.run()` appended only a `role="tool"` result message after executing an LLM-requested Tool call, never the preceding `role="assistant"` message carrying the model's own `tool_calls` — a real Azure OpenAI/OpenAI protocol requirement (`"messages with role 'tool' must be a response to a preceding message with 'tool_calls'"`). `MockLLMProvider` never exposed it because it never serializes `request.messages` to any external API — it's a pure in-process text-derivation function with no concept of message-sequencing validity. **Minimum fix:** `src/llm/models.py`'s `LLMMessage` gained one new field, `tool_calls: list[ToolCallRequest] | None = None` (set only on ASSISTANT messages representing a tool-call request) — the missing typed vocabulary that made a compliant message unconstructable; `ToolCallArgument`/`ToolCallRequest` were reordered earlier in the file so the new field could reference `ToolCallRequest` directly. `ToolCallingOrchestrator.run()` now appends exactly one ASSISTANT message (carrying the complete, ordered set of that turn's `tool_calls`) immediately before the TOOL result message(s) — provider-agnostic, unconditional whenever tool_calls were requested (covering the unauthorized/unknown/malformed/execution-failure cases too, not just the success path). `AzureOpenAIProvider._to_openai_messages` and `OllamaLLMProvider._to_ollama_messages` each gained the matching mapping branch for their own wire shape (OpenAI: `{role:"assistant", tool_calls:[{id, type, function}]}`; Ollama: no `id`, arguments as a plain object) — no special-casing, both providers updated consistently, matching the explicit "do not special-case Azure OpenAI" instruction. `MockLLMProvider` needed zero changes. **Tests:** 2 new + 5 strengthened tests in `test_tool_calling_orchestrator.py` (assistant-tool_calls persistence, tool_call_id linkage, multi-tool-call ordering within one turn, ordering across iterations, unauthorized/unknown/malformed/max-iteration sequencing, plus a dedicated real-`MockLLMProvider` end-to-end regression test), 3 new tests in `test_azure_openai_provider.py` (SDK-shape mapping, text-preserved-alongside-tool_calls, plain-assistant-message regression guard), 1 new test in `test_ollama_provider.py`. Full suite: 468 backend tests pass (2 skipped, +7 from the 461 baseline), zero regressions; ruff and mypy clean; all Broker/Commercial/Claims-tool-calling-integration/full-chat-API test files re-run explicitly (133 passed). **Deployment:** only `ca-tmxap-dev-api` was rebuilt (new versioned tag) and updated — `ca-tmxap-dev-web` and every other resource confirmed untouched (revision names unchanged, all 12 resources still present). **Live validation** (see `validation.md`): the exact 2-turn Claims sequence that previously returned `500` now returns `200` with a real, successful `policy_lookup` Tool call; the same conversation was driven a full 10 turns to completion against the real `gpt-5-mini` deployment, ending in a genuine `claim_reference: "SYN-CLM-2026-0001"` and a real adjuster assignment — the complete Claims workflow, including multiple real Tool calls, now works end-to-end against real Azure OpenAI for the first time. Broker and Commercial each independently re-verified live with a full 2-turn conversation and real Tool-sourced business data, confirming zero regression. STOP CONDITION: all items met. No Agent, Supervisor, PromptManager, RAG, Grounding, citation, Tool-allow-list, frontend, or Azure infrastructure code touched. — 2026-08-08
Evidence: `docs/sprint_04/evidence/pbi-04-03-tool-calling-message-sequence-fix-validation.txt`
